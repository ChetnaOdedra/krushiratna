...
const BuyerSellingStatus = (Props) => {
  ...
  const selectImage = () => {
    Alert.alert(t('Upload Bill'), t("Choose an option"), [
  ...
  function ComplateOrder() {
    ...
    let params = {final_price: price, final_weight: weight, media_id: MidiaImageID, order_id: id};
    console.log("BuyerComplateOrder Params:", params);
    api.BuyerComplateOrder(params).then((res) => {
      console.log("Full Response:", res); // Log full response
      console.log("BuyerComplateOrder Response Data:", res.data); // Log data from response
  ...
  return (
    <View style={{flex:1}}>
         <BuyerCustomHeader name={UesrMob.user.user.name} role={UesrMob.user.user.role} />
        <View style={styles.container}>
      {/* Title */}
       <Text style={{fontSize:22,color:'black',marginTop:10,fontWeight:'600',marginBottom:20}}>{t('Selling Status')}</Text>
      {/* Final Weight */}
      <View style={styles.inputContainer}>
        <Text style={styles.label}>{t('Final Weight')}</Text>
        <TextInput
          style={styles.input}
          value={weight}
...
export default BuyerSellingStatus;
