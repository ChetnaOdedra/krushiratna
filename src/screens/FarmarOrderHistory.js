...
const BuyerOrderHistory = (props) => {
  ...
  function GetListOrderHistory() {
    let params = {
      status: activeTab,
      order_status_id: activeTab === 'Completed' ? 4 : 5,
    };
     api.FarmerGetOrderHistory(params).then((res) => {
      if (res.ok) {
        setData(res.data.data);
        console.log("Order History Data:", res.data.data);
        showToast(t('Get Order History successfully'));
      } else {
        showToast(t('Get Order History failed'));
      }
    }).catch((error) => {
      showToast(t('Get Order History failed'));
    });
